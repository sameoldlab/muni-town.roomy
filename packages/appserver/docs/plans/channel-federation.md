...
**Status:** Phases 1-4 **shipped** (backend relationship lifecycle, origin + receiver grants, federated read/write, frontend). Phase 5 hardening **shipped** (revoke cleanup, B-side removal, double-request guard, banned-member denial, invalidation on grant changes, deleted-channel sidebar skip). **Sidebar reorder fix shipped**: federated channels placed in a category via drag-and-drop reorder now stay there (see §0).
**Gating:** none — the `channel-federation` feature flag was **removed** once the feature shipped permanently (the UI is unconditional; the registry entry is gone).
...
- **Feature flag removed:** `channel-federation` was registered in `FEATURE_FLAGS` (`featureFlags.ts`) and returned by `space.roomy.getFlags` while the feature was rolling out. Once federation shipped permanently the gate was removed: the registry entry is gone and the three app-lite read sites (federation settings tab, pending-request badge, federations page) are unconditional. Server-side event processing was never flag-guarded — only the frontend UI was.
...
All frontend work was gated on the `channel-federation` feature flag (returned by `space.roomy.getFlags`) during the rollout; that flag has since been removed and the federation UI is unconditional (see §0). Server-side event processing was never flag-guarded.
...
### Phase 4 — Frontend ✅ shipped
- Frontend was gated on the `channel-federation` flag during rollout; the flag was removed after the feature shipped permanently (see §0), so all UI below is unconditional.
...
3. **Feature flag safety.** *Resolved — the flag was removed.* Planning assumed the flag would also be enforced server-side so frontend-only gating wasn't a security hole. In the end no server-side gate was ever added, and the UI gate has since been removed outright, so federation is unconditional on both sides. Access is governed solely by the grant chain (origin grant → receiver grant), which is the correct authority.
