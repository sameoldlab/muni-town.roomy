...
> = async (params: QueryParams, auth: AuthCtx) => {
  ...
  return withSpan(
    ...
    async (span) => {
      ...
      await withSpan("getThreads.requireRead", {}, () =>
        requireSpaceRead(db, spaceId, userDid, memo),
      );

      // The index board shows channels AND threads, ordered by activity.
      const { threads: all, cursor: nextCursor } = await withSpan(
        "getThreads.listActivity",
        {},
        async (s) => {
          const result = await listThreadActivity(
            db,
            { kind: "space", spaceId },
            limit,
            cursor,
          ...
          s.setAttribute("roomy.thread_count", result.threads.length);
          return result;
        },
      );
      span.setAttribute("roomy.room_count", all.length);
      ...
      // DB round-trips). Thread visibility hangs off the canonical parent
      // channel — the auth unit computes that via the 'link' edge, matching
      // the spec's "channel grants visibility" model.
      const accessByRoom = await withSpan("getThreads.roomAccess", {}, () =>
        roomAccessMany(db, all.map((t) => t.id), userDid, memo),
      );
      ...
      span.setAttribute("roomy.visible_room_count", rooms.length);
