<script lang="ts">
  ...
  function highlightSearchTerm(content: string, searchTerm: string): string {
    ...
    return content.replace(regex, '<mark class="bg-primary/30 text-base-content">$1</mark>');
  }
...
<div class="search-results bg-base-100 border border-base-300 rounded-lg shadow-lg w-full max-h-[60vh] overflow-auto">
  <div class="sticky top-0 bg-base-100 p-3 border-b border-base-300 flex justify-between items-center z-10">
    <h3 class="font-bold text-base-content">
      Search Results {#if messages.length > 0}<span class="text-sm font-normal">({messages.length})</span>{/if}
    </h3>
            ...
            {#await getProfile(message.authors((x) => x.get(0))) then profile}
              ...
              <div class="flex-1 min-w-0">
                <div class="flex justify-between items-center mb-1">
                  <span class="font-medium text-base-content">{profile.handle || "Unknown"}</span>
                  <span class="text-xs text-base-content/60">
                    {message.createdDate ? formatDistanceToNow(message.createdDate, { addSuffix: true }) : ""}
                  </span>
                </div>
